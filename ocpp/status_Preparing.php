...
cpLog("status_Preparing.php pokrenut (cp_id={$cpId}, user_id=" . ($userId ?? 'NULL') . ")", $status, $cpId);
...
try {
    ...
    cpLog("OCPP Preparing frame poslan na $wsUrl", $status, $cpId);
...
cpLog("Upisan novi status 'Preparing' u Status_Punionice", $status, $cpId);
...
cpLog("status_Preparing.php završen", $status, $cpId);
