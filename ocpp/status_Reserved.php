...
cpLog("status_Reserved.php pokrenut (cp_id={$cpId}, user_id=" . ($userId ?? 'NULL') . ")", $status, $cpId);
...
try {
    ...
    cpLog("OCPP Reserved frame poslan na $wsUrl", $status, $cpId);
...
cpLog("Upisan novi status 'Reserved' u Status_Punionice", $status, $cpId);
...
cpLog("status_Reserved.php završen", $status, $cpId);
