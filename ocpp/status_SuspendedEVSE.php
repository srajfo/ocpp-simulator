...
cpLog("status_SuspendedEVSE.php pokrenut (cp_id={$cpId}, user_id=" . ($userId ?? 'NULL') . ")", $status, $cpId);
...
try {
    ...
    cpLog("OCPP SuspendedEVSE frame poslan na $wsUrl", $status, $cpId);
...
cpLog("Upisan novi status 'SuspendedEVSE' u Status_Punionice", $status, $cpId);
...
cpLog("status_SuspendedEVSE.php završen", $status, $cpId);
