...
cpLog("status_SuspendedEV.php pokrenut (cp_id={$cpId}, user_id=" . ($userId ?? 'NULL') . ")", $status, $cpId);
...
try {
    ...
    cpLog("OCPP SuspendedEV frame poslan na $wsUrl", $status, $cpId);
...
cpLog("Upisan novi status 'SuspendedEV' u Status_Punionice", $status, $cpId);
...
cpLog("status_SuspendedEV.php završen", $status, $cpId);
