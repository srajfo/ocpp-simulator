...
cpLog("status_Unavailable.php pokrenut (cp_id={$cpId}, user_id=" . ($userId ?? 'NULL') . ")", $status, $cpId);
...
try {
    ...
    cpLog("OCPP Unavailable frame poslan na $wsUrl", $status, $cpId);
...
cpLog("Upisan novi status 'Unavailable' u Status_Punionice", $status, $cpId);
...
cpLog("status_Unavailable.php završen", $status, $cpId);
