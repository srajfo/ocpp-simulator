...
$rights = getUserRights($_SESSION['user_id']);
...
function getUserStatusHistory(mysqli $mysqli, int $idPunionice, int $userId, int $limit = 10): array {
    $stmt = $mysqli->prepare("
        SELECT sp.ID_Status, sp.VrijemePostavljanja, sp.ID_Korisnika, s.Naziv
        FROM Status_Punionice sp
        JOIN Status s ON s.ID = sp.ID_Status
        WHERE sp.ID_Punionice = ? AND sp.ID_Korisnika = ?
        ORDER BY sp.VrijemePostavljanja DESC
        LIMIT ?
    ");
    $stmt->bind_param('iii', $idPunionice, $userId, $limit);
    $stmt->execute();
...
<?php foreach ($punionice as $p):
    ...
    $userHistory = getUserStatusHistory($mysqli, $p['ID'], $uid);
    $status = getCurrentStatus($mysqli, $p['ID']);
    $uiState = mapStatusToUiState($status);
    ...
    if ($transaction && isset($transaction['id'])) {
        ...
        if ($socRow) {
            $soc = intval($socRow['soc']);
        }
    }

    // -----------------------------
    // ENABLE/DISABLE LOGIKA GUMBA
    // -----------------------------
...
<div class="charger" data-id="<?= esc($p['ID']) ?>">
    ...
    <div class="charger-right">
        ...
        <?php if ($userHistory): ?>
            <div class="smalltag">Vaši zadnji statusi:</div>
            <table class="table table-striped">
                ...
                <tbody>
                    <?php foreach ($userHistory as $r): ?>
                        <tr>
                            <td><?= esc($r['VrijemePostavljanja']) ?></td>
                            <td><?= esc($r['Naziv']) ?></td>
                            <td><?= esc($r['ID_Korisnika']) ?></td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
        <?php else: ?>
            <div class="smalltag">Nemate statusa za ovu punionicu.</div>
        <?php endif; ?>
...
<?php
$color = "black";
if ($status === "Charging") $color = "green";
if ($status === "Available") $color = "blue";
if ($status === "SuspendedEVSE" || $status === "SuspendedEV") $color = "orange";
if ($status === "Faulted") $color = "red";
if ($status === "Finishing") $color = "purple";
if ($status === "Preparing") $color = "gray";
if ($status === "Reserved") $color = "brown";
if ($status === "Unavailable") $color = "darkred";
?>
